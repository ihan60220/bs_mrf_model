...
class MrfRec(object):
    ...
    def initializeBand(self, kx, ky, Eb, offset=0., flipKAxes=False, kScale=1., interp_method='linear'):
        ...
        **Parameters**\n
        ...
        intFunc = interpolate.RegularGridInterpolator((kx_in, ky_in), Eb_in, method=interp_method, bounds_error=False,
                                                      fill_value=None)
    ...
    def normalizeI(self, kernel_size=None, n_bins=128, clip_limit=0.01, use_gpu=True, threshold=1e-6):
        ...
        **Parameters**\n
        ...
        try:
            ...
            self.I = mclahe(self.I, kernel_size=kernel_size, n_bins=n_bins, clip_limit=clip_limit, use_gpu=use_gpu)
        except:
    ...
    def symmetrizeI(self, mirror=True, rotational=True, rotational_order=6):
        ...
        **Parameters**\n
        ...
        if mirror:
            ...
            indXRef = np.min(np.where(self.kx > 0.)[0])
            ...
            indYRef = np.min(np.where(self.ky > 0.)[0])
    ...
    def iter_seq(self, num_epoch=1, updateLogP=False, disable_tqdm=False):
        ...
        **Parameters**\n
        ...
        for i, ind in enumerate(tqdm(indList, disable=disable_tqdm)):
            ...
            if updateLogP and (((i + 1) % (self.lengthKx * self.lengthKy)) == 0 or ((i + 1) % (self.lengthKx * self.lengthKy)) == (self.lengthKx * self.lengthKy // 2)):
    ...
    def iter_para_curv(self, num_epoch=1, updateLogP=False, use_gpu=True, disable_tqdm=False, graph_reset=True, **kwargs):
        ...
        **Parameters**\n
        ...
        indEb = [[tf.Variable(np.expand_dims(self.indEb[indX + i, indY + j], 2), dtype=tf.int32) for j in range(3)] for i in range(3)]
        ...
        for i in range(3):
            for j in range(3):
                ...
                logP[i][j] = logI[i][j]\
                             - tf.pad(tf.slice(squDiff[i - 2][j], [pi[1], 0, 0], [nKx - pi[1], nKy, nE]), [[0, pi[1]], [0, 0], [0, 0]])\
                             - tf.pad(tf.slice(squDiff[i][j - 2], [0, pj[1], 0], [nKx, nKy - pj[1], nE]), [[0, 0], [0, pj[1]], [0, 0]])\
                             - tf.pad(tf.slice(squDiff[i - 1][j], [0, 0, 0], [nKx - pi[0], nKy, nE]), [[pi[0], 0], [0, 0], [0, 0]])\
                             - tf.pad(tf.slice(squDiff[i][j - 1], [0, 0, 0], [nKx, nKy - pj[0], nE]), [[0, 0], [pj[0], 0], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(tf.roll(EbCurv[i - 2][j], shift=1-pi[1], axis=0)
                                                         - 2 * tf.roll(EbCurv[i - 1][j], shift=pi[0], axis=0) + ECurv3d,
                                                         [1 - pi[1], 0, 0], [nKx - 1 + pi[1], nKy, nE])),
                                      [[1 - pi[1], 0], [0, 0], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(tf.roll(EbCurv[i - 1][j], shift=pi[0], axis=0) - 2 * ECurv3d
                                                         + tf.roll(EbCurv[i - 2][j], shift=-pi[1], axis=0),
                                                         [pi[0], 0, 0], [nKx - pi[0] - pi[1], nKy, nE])),
                                      [[pi[0], pi[1]], [0, 0], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(ECurv3d - 2 * tf.roll(EbCurv[i - 2][j], shift=-pi[1], axis=0)
                                                         + tf.roll(EbCurv[i - 1][j], shift=pi[0]-1, axis=0),
                                                         [0, 0, 0], [nKx - 1 + pi[0], nKy, nE])),
                                      [[0, 1 - pi[0]], [0, 0], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(tf.roll(EbCurv[i][j - 2], shift=1-pj[1], axis=1)
                                                         - 2 * tf.roll(EbCurv[i][j - 1], shift=pj[0], axis=1) + ECurv3d,
                                                         [0, 1 - pj[1], 0], [nKx, nKy - 1 + pj[1], nE])),
                                      [[0, 0], [1 - pj[1], 0], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(tf.roll(EbCurv[i][j - 1], shift=pj[0], axis=1) - 2 * ECurv3d
                                                         + tf.roll(EbCurv[i][j - 2], shift=-pj[1], axis=1),
                                                         [0, pj[0], 0], [nKx, nKy - pj[0] - pj[1], nE])),
                                      [[0, 0], [pj[0], pj[1]], [0, 0]])\
                             - tf.pad(tf.square(tf.slice(ECurv3d - 2 * tf.roll(EbCurv[i][j - 2], shift=-pj[1], axis=1)
        ...
        if (plotBand or plotBandInit) and (E is None):
            ...
            if plotSliceInBand:
                ...
                if equal_axes:
                    ...
                    ax.set_aspect('equal', 'box')
                if kx is not None:
                    plt.plot(np.array(self.lengthKy * [self.kx[indKx]]), self.ky, 'r', linewidth=2.0)
                else:
    ...
    def plotLoss(self):
        ...
        fig = plt.figure()
        ax = fig.gca()
        ax.plot(epoch, -self.logP, linewidth=2.)
        ax.set_xlabel('epochs', fontsize=24)
        ax.set_ylabel('logp', fontsize=24)
